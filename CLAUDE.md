...
# Правило: миграции БД при изменении схемы

## Когда добавляешь колонку или таблицу

1. Добавь `CREATE TABLE IF NOT EXISTS` / колонку в `initSchema()` — для новых БД
2. Добавь миграцию в `migrateSchema()` — для существующих БД (проверка через `PRAGMA table_info` + `ALTER TABLE`)
3. **Сразу после сохранения `db.ts`** — выполни миграцию вручную, чтобы текущая БД обновилась:
   ```
   node -e "const Database = require('better-sqlite3'); const db = new Database('data/brain.db'); <миграция>"
   ```
4. Убедись, что API отдаёт 200 после изменения: `curl -s -o /dev/null -w '%{http_code}' http://localhost:3000/api/items`

**Почему:** dev-сервер может не перезапустить singleton `db` при hot-reload, и миграция не отработает автоматически. Ручной запуск гарантирует, что БД соответствует коду.
