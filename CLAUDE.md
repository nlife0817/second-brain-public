...
- [src/components/v2/sheet/](src/components/v2/sheet/) — редактор таблиц базы знаний: полотно с виртуализацией, протягивание за уголок, меню по правой кнопке, строка формул, панель оформления, вкладки листов. Чистое ядро (модель книги, формулы, ряды протягивания, форматы, разбор xlsx и csv) — в [src/lib/core/sheet/](src/lib/core/sheet/)
...
Новая миграция — просто файл `supabase/migrations/NNNN_<name>.sql` (следующий свободный номер — **0059**). Выкат применяет непринятые файлы сам, по одному, каждый в своей транзакции; учёт — в `public._deploy_migrations`. Подробности и ручные режимы — [docs/DEPLOY.md](docs/DEPLOY.md).
