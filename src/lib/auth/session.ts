// Сессия: подписанная HMAC-SHA256 cookie.
//
// Внешнего сервиса за ней нет: приложение само проверяет пароль
// (lib/auth/password.ts) и само подписывает cookie.
//
...
// только WebCrypto, доступный в обоих. Пароль здесь не проверяется никогда:
// на Edge приезжает уже подписанное значение.
//
// Безопасность: cookie подписана, но не зашифрована — внутрь кладём только то,
// что и так известно владельцу браузера (id, email, имя). Ни пароля, ни его
// хеша тут нет. Права по этим данным не выдаются: доступ проверяется отдельным
// запросом к `core.org_members` на каждый запрос — исключённый из организации
// теряет его сразу, не дожидаясь истечения cookie.
...
export interface SessionUser {
  /**
   * `core.users.id`. До перехода на пароли здесь лежал числовой `sub` от Google —
   * выданные тогда cookie доживают свои 30 дней, поэтому разрешение
   * пользователя идёт по email (см. lib/core/context.ts), а не по этому полю.
   */
  id: string;
  email: string;
  fullName: string;
 ...
 *
 * sameSite: "lax", а не "strict" — при "strict" браузер не прикладывает cookie
 * к навигации со стороннего сайта, и переход по ссылке на задачу из мессенджера
 * или письма приводил бы на экран входа, хотя человек уже вошёл.
 */
