...
import { getDirection, updateDirection, deleteDirection } from "@/lib/db";
import { logChange, buildDiff } from "@/lib/planning-changelog";

export const GET = withAuth(async (_req, ctx) => {
  const { id } = await ctx.params;
  const row = await getDirection(id);
...
export const PATCH = withAuth(async (req: NextRequest, ctx, user) => {
  const { id } = await ctx.params;
  const before = await getDirection(id);
  if (!before) return NextResponse.json({ error: "not found" }, { status: 404 });
  const body = await req.json();
  const after = await updateDirection(id, body);
...
export const DELETE = withAuth(async (_req, ctx, user) => {
  ...
  await logChange({ actor_email: user.email, entity_type: "direction", entity_id: id, action: "delete" });
