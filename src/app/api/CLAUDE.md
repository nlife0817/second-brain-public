...
Роуты `/api/v2/**` дополнительно оборачиваются `withOrg`/`withUser` из [lib/core/context.ts](../../lib/core/context.ts) — они и разрешают пользователя, и проверяют членство в организации. Роуты v1 полагаются на проверку сессии в proxy; ролевые ограничения — в [lib/api-auth.ts](../../lib/api-auth.ts).
