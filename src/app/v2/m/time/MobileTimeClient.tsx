...
/** Данные с сервера вместе с периодом, за который он их посчитал. */
export interface MobileTimeInitial {
  from: string;
  to: string;
...
export function MobileTimeClient({ initial }: { initial: MobileTimeInitial }) {
  ...
  // Серверный ответ — в кэш под ключом того периода, который посчитал сервер.
  // Совпал с местным (обычный случай) — первого запроса не будет; разошёлся на
  // сутки из-за часового пояса — экран догрузит правильный.
  useEffect(() => {
    if (orgId) seed(`/orgs/${orgId}/time?from=${initial.from}&to=${initial.to}`, initial.list);
  }, [orgId, initial]);
  ...
  const load = useCallback(
    async (opts: { force?: boolean } = {}) => {
      if (!orgId) return;
      try {
        const res = await cachedGet<{ entries: TimeEntry[]; active: TimeEntry | null }>(
