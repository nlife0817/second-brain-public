...
create table core.calendars (
  ...
  -- приходится.
  --
  -- Инкрементального токена Google (`syncToken`) здесь намеренно нет. Он
  -- несовместим с `timeMin`/`timeMax`, а без временного окна `singleEvents=true`
  -- разворачивает повторяющиеся события за всю историю календаря — то есть
  -- вместо экономии даёт неограниченную выдачу. Мы синхронизируем скользящее
  -- окно (см. SYNC_WINDOW в lib/core/calendars.ts) целиком: оно ограничено по
  -- построению, и его повторный разбор дешевле, чем риск неполной выборки.
  http_etag    text,
  last_sync_at timestamptz,
  created_at   timestamptz not null default now(),
  updated_at   timestamptz not null default now(),
