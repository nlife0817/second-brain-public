...
-- ############################################################################
--
-- Маркер `deploy: manual` в первой строке держит эту миграцию вне автовыката:
-- deploy.sh её пропускает и остаётся зелёным. Запуск — только руками, после
-- предполётной проверки ниже:
--     /srv/secondbrain/deploy/migrate.sh --run 0037_drop_v1_public.sql
--
-- НА VPS ФАЙЛ В ТЕКУЩЕМ ВИДЕ НЕ ВЫПОЛНИТСЯ. Он писался под Supabase, а прод
-- переехал на голый Postgres в контейнере: раздела 2 (`cron.job`, `cron.unschedule`)
-- там нет вовсе — схема `cron` не существует, и блок упадёт с undefined_table.
-- Расписания v1 на VPS отключены иначе: они закомментированы в deploy/cron/crontab.
-- Раздел 3 (публикация supabase_realtime) отработает вхолостую — публикации нет.
-- Перед прогоном оба раздела нужно убрать, оставив только DROP-ы.
--
